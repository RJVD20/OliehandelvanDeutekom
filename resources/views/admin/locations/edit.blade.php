...
<form method="POST" action="{{ route('admin.locations.update', $location) }}" class="space-y-6 max-w-2xl">
    @csrf
    @method('PUT')

    <div>
        <label class="text-sm font-medium">Naam</label>
        <input type="text" name="name" value="{{ old('name', $location->name) }}" class="w-full rounded-lg border px-3 py-2" required>
    </div>

    <div>
        <label class="text-sm font-medium">Straat</label>
        <input type="text" name="street" value="{{ old('street', $location->street) }}" class="w-full rounded-lg border px-3 py-2">
    </div>

    <div>
        <label class="text-sm font-medium">Postcode + plaats</label>
        <input type="text" name="postcode_city" value="{{ old('postcode_city', $location->postcode_city) }}" class="w-full rounded-lg border px-3 py-2">
    </div>

    <div>
        <label class="text-sm font-medium">Openingstijden</label>
        <textarea name="opening" rows="4" class="w-full rounded-lg border px-3 py-2">{{ old('opening', $location->opening) }}</textarea>
    ...
    <div>
        ...
        <input type="text" name="phone" value="{{ old('phone', $location->phone) }}" class="w-full rounded-lg border px-3 py-2">
    </div>

    <div class="grid grid-cols-2 gap-4">
        <div>
            <label class="text-sm font-medium">Latitude</label>
            <input type="text" name="lat" value="{{ old('lat', $location->lat) }}" class="w-full rounded-lg border px-3 py-2">
        </div>
        <div>
            <label class="text-sm font-medium">Longitude</label>
            <input type="text" name="lng" value="{{ old('lng', $location->lng) }}" class="w-full rounded-lg border px-3 py-2">
        </div>
    </div>
    ...
    <div>
        ...
        <input type="text" name="remark" value="{{ old('remark', $location->remark) }}" class="w-full rounded-lg border px-3 py-2">
    </div>
